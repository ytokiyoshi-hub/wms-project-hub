...
| 選択肢C | 先入先出法（FIFO 評価） |
| 選択肢D | 最終仕入原価法 |
| **時吉さん回答** | **A**（2026-05-14 推奨採用：移動平均法） |
...
| **時吉さん回答** | **B**（2026-05-14 推奨採用：荷主×ロール+RLS） |
...
## 決定事項サマリー（2026-05-14 全項目確定）

| 論点 | 決定内容 | 致命傷ライン該当 | ファイル |
|------|----------|----------------|---------|
| BF-1：検品・ピッキング順序 | **確定済（D=荷主切替）** ✓ | BF-1 | process_02_inbound.md Q2 |
| BF-2：引当ロジック | **D：多軸+荷主切替** ✓ | BF-2 / DB-1 / DB-2 / DB-5 | 本ファイル |
| BF-3：ピッキング方式 | **D：荷量×荷主切替** ✓ | BF-3 / LK-2 | 本ファイル |
| BF-4：返品・誤出荷処理 | **確定済（B=在庫に載せず別ステ）** ✓ | BF-4 | process_02_inbound.md Q5 |
| CA-1：請求賃率計算 | **D：全方式+荷主切替** ✓ | CA-1 | 本ファイル |
| CA-2：原価評価方式 | **A：移動平均法** ✓ | CA-2 | 本ファイル |
| AU-1：権限・承認フロー | **B：荷主×ロール+RLS** ✓ | AU-1 / DB-4 | 本ファイル |
| AU-2：SKU/JANコード管理 | **D：荷主×内部SKU+JAN** ✓ | AU-2 / LK-2 / DB-4 | 本ファイル |
| LK-1：外部連携方式 | **D：CSV+API+EDI 切替** ✓ | LK-1 | 本ファイル |
| LK-2：HTバーコード仕様 | **D：荷主マスタで読取項目** ✓ | LK-2 / DB-3 / AU-2 | 本ファイル |
