...
設計方針：
- `owners` テーブルに `jan_strict BOOLEAN NOT NULL DEFAULT true` を追加
- `jan_strict = true` の荷主は `^[0-9]{8}$` または `^[0-9]{13}$` のアプリ側バリデーションを適用
- `jan_strict = false` の荷主は書式チェックなし（荷主独自コード許容）
- DB の CHECK 制約は追加しない（荷主ごとの設定差異を DB 制約では表現できないため）

```sql
ALTER TABLE owners ADD COLUMN jan_strict BOOLEAN NOT NULL DEFAULT true;
```
...
CREATE TABLE unit_master (
  id         BIGSERIAL PRIMARY KEY,
  unit_code  TEXT NOT NULL UNIQUE,   -- 'case', 'piece', 'hour' 等
  unit_label TEXT NOT NULL,          -- 表示名（日本語）
...
-- 初期データ投入
INSERT INTO unit_master (unit_code, unit_label, sort_order) VALUES
  ('case',      'ケース',       1),
  ('piece',     'ピース',       2),
  ('tsubo',     '坪',           3),
  ('sqm',       '平方メートル', 4),
...
ALTER TABLE billing_rules ADD CONSTRAINT billing_rules_unit_fk
  FOREIGN KEY (unit) REFERENCES unit_master(unit_code);
```
