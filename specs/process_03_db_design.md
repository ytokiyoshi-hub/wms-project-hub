...
| **時吉さん回答** | **B**（2026-05-14 推奨採用・★最重要・全テーブル前提） |
...
## 決定事項サマリー（2026-05-14 確定）

| 論点 | 決定内容 | 致命傷ライン該当 |
|------|----------|----------------|
| DB-1：在庫の持ち方 | **D：4軸（荷主×SKU×ロット×ロケ）** | DB-1 / 全 DB に波及 |
| DB-2：ロット管理方式 | **D：荷主切替（荷主マスタフラグで B/C 切替）** | DB-2 / BF-2 |
| DB-3：シリアル管理方式 | **B：SKUフラグ（serial_required=true のみ）** | DB-3 / LK-2 |
| DB-4：荷主切替の方式 | **B：論理分離+RLS（全テーブル owner_id 必須）** | DB-4 / AU-1 / 差別化4点 |
| DB-5：ロケーション管理方式 | **D：階層×機能種別（abc_class + location_type）** | DB-5 / BF-2 / BF-3 / LK-2 |

---

## 未解決の論点

- なし（2026-05-14 全項目確定済み）
- 今井先生レビューは並行受領で進行（覆る場合は `restore-point-2026-05-14` に戻して再判断）
