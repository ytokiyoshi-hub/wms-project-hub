...
## 「触れる本番品質」達成基準（時吉さん要望ベース）

✓ サイドバーがエンドユーザー視点で使える（業界標準6カテゴリ）  
✓ 画面開いて中身データが見える（実BULK 21k件 + 業務拡充データ）  
✓ 業務として1日通せる（入荷検品→棚入→ピック→検品→梱包→積込→引渡→請求）  
✓ 公開URLで触れる（GitHub Pages）  
✓ HT端末視点でも動く（KEYENCE BT-A2000 4インチ模型 + 業務フロー）  
✓ 帳票が業務として印刷可能なフォーマット  
✓ フィルタで業務的絞り込みが動く
