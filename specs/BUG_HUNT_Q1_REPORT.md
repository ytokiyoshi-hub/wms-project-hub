...
## 3. ❌ CRITICAL: 欠損・破損 API JSON ファイル

### 3-1. api/inventory/lot-trace.json 欠損 (NEW: 2026-05-29 追加)

`pc/inventory/lot_traceability.html` と `pc/inbound/plan_line_detail.html` が GET `/api/inventory/lot-trace?...` を呼び出すが、**対応JSONファイルが存在しない**。

| 影響ページ | 症状 |
|---------|------|
| `pc/inventory/lot_traceability.html` | WMS Bridge フォールバック: `api/inventory.json` の先頭1件を返す → ロット不一致データが表示される |
| `pc/inbound/plan_line_detail.html` | 同上 + `__wms_errs` に `{api404: "inventory/lot-trace"}` 記録 |

**修正方針**: `api/inventory/lot-trace.json` を新規作成（ロットトレーサビリティ用サンプルデータ）

---

### 3-2. 破損 API JSON ファイル (HTML 404 ページが混入)
...
### ❌ 修正必須
1. **api/inventory/lot-trace.json** → 新規作成（ロットトレーサビリティ用サンプルデータ）← 2026-05-29 追加
2. **api/csv-format-masters.json** → 正しいCSVフォーマット定義JSONに差し替え
3. **api/inventory-snapshots.json** → 在庫スナップショットJSONデータに差し替え（または `[]`）
4. **api/stocktake-sessions.json** → 棚卸セッションJSONデータに差し替え（または `[]`）
5. **api/inventory/move.json** → 在庫移動JSONデータに差し替え（または `[]`）
